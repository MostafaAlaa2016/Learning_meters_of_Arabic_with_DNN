...
\begin{figure*}[!t]
  ...
  \caption{Different encoding mechanisms%: \textit{\textit{one-hot}}, \textit{\textit{binary}}, and \textit{\textit{two-hot}}  encoding. Using the word example %\textarabic{مَرْحَبَا} \textit{\textit{one-hot}} encoding is applied using $n$-letter alphabet $n = 181$ in Arabic. Same word encoded using \textit{\textit{binary}} encoding where the vector length $n = \ceil*{\log_2 l}$, $l \in \{181\}$. \textit{\textit{two-hot}} encoding is applied by stacking $\bm k_{4 \times 1}$ on the top of $\bm m_{37 \times 1}$ we get the $\textit{two-hot}_{41 \times 1}$ \usebox{\columnVector}, which represents a letter and its diacritic simultaneously. 
