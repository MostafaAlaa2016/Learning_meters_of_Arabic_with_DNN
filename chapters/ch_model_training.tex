...
The second Interesting example to check how the model can understand the pattern without \textit{Tashkeel} and the effect of adding \textit{Tashkeel} to the text. We will take the second row from Table~\ref{Tab:Results_Article} and check the effect of adding and removing \textit{Tashkeel}. If we find the sentence without Tashkeel it follows the pattern of \textit{Al-Taweel} meter. Although the text meanings lead to another classification which is in this case not related to the 16 classic meters. But the meanings was not clear to the Model. So, the model tried to find the nearest meter to the sequences of the letters.

The result was very impressive when we classify the sentence without \textit{Tashkeel} it classified to \textit{Al-Taweel} with score 0.9988 but when we add the \textit{Tashkeel} to the sentence it classified to not a poem with score 0.8390. This effect show how the model can highly utilize the \textit{Tashkeel} as feature to get more details and this was explained previous. 
