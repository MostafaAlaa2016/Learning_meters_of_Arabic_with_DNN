...
\caption{Different encoding mechanisms: \textit{One-hot}, \textit{binary}, and
  \textit{two-hot} encoding. using the word example \textarabic{مَرْحَبَا}. using $n$-letter alphabet
  \textit{one-hot} encoding; the \textit{one-hot} is applied to encode Arabic and English verses;
  the only difference is the vector length is 181 in Arabic and is 28 is English. In binary
  encoding, the same word is encoded using \textit{binary} encoding, where the vector length
  $n = \ceil*{\log_2 l}$, $l \in \{181, 28\}$ is the alphabet length (Arabic/English). By stacking
  $\bm k_{4 \times 1}$ on the top of $\bm m_{37 \times 1}$ we get the
  $Two-Hot_{41 \times 1}$\usebox{\columnVector}, which represents a letter and its diacritic,
  simultaneously.}\label{fig:One-Binary-Encoding}
\end{figure*}
