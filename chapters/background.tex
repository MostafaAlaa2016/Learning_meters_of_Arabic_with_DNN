...
\end{figure}
 ...
 Now, we will represent the functions per layer as below equations \eqref{eq:nn_multi_layer}
 
\begin{equation}\label{eq:nn_multi_layer}
  \boxed{z^l =  W^l x + b^l} \longrightarrow \boxed{a_i^l =  \sigma(z^l)}  \longrightarrow \boxed{\ell(a^l,y)} \quad \text{\textit{l is refer to layer number, i refer to the node number in the layer}}
\end{equation}
